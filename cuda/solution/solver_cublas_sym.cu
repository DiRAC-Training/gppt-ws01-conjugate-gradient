...
// Dense matrix-vector product: y = A * x. A is symmetric and fully stored, so
// Ssymv reads one triangle and the row-major/column-major transpose is a no-op;
// halves matrix traffic.
void matvec(real *y, const real *A, const real *x, const int n) {
  const real one = 1.0f, zero = 0.0f;
#ifdef SINGLE_PRECISION
  cublasSsymv(cublas.handle, CUBLAS_FILL_MODE_UPPER, n, &one, A, n, x, 1, &zero,
              y, 1);
#else
  cublasDsymv(cublas.handle, CUBLAS_FILL_MODE_UPPER, n, &one, A, n, x, 1, &zero,
              y, 1);
#endif
